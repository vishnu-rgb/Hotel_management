import random                #gerate a random key for user id 
import datetime              #  provide date and time 
#Code present by Vishnu and Nitish .........Branch "ECE" 
...
#Use date formate as day/month/year
def date(c): 
	
	if c[2] >= 2020 and c[2] <= 2030:   # check for year 
		
		if c[1] != 0 and c[1] <= 12:    # check for months 
			
			if c[1] == 2 and c[0] != 0 and c[0] <= 31:   # check for days 
...
def Booking(): 
		...
		cii=str(input("Check-In: { use date format as dd/mm/yr }")) 
		checkin.append(cii) 
		cii=cii.split('/') 
		ci=cii 
		ci[0]=int(ci[0]) 
...
def restaurant(): 
	...
	for n in range(0,i): 
		if custid[n]==ph and p[n]==0: 
			f=1
            # Menu list wuth price mention along side
			print("-------------------------------------------------------------------------") 
			print("						 Hotel ALISHA") 
			print("-------------------------------------------------------------------------") 
			print("						 Menu Card") 
